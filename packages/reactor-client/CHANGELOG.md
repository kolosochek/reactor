...
- `ReactorClient` class - HTTP client over `@dkolosovsky/reactor-service`. Pure fetch-based; browser- and Node-friendly. Constructor normalizes `baseUrl` (trailing slash stripped) and accepts an injectable `fetchImpl` for testing.
...
- Re-exports `buildCoverLetterIdea` / `buildScoreIdea` / `buildQuestionsIdea` plus their input/solution Zod shapes from `@dkolosovsky/reactor-text-tools` so consumers get the whole surface from a single import path.
